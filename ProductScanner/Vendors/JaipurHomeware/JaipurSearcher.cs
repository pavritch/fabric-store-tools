...
namespace JaipurHomeware
{
    ...
    {
        private readonly IVendorScanSessionManager<JaipurHomewareVendor> _sessionManager;
        private readonly IPageFetcher<JaipurHomewareVendor> _pageFetcher;

        public JaipurSearcher(IVendorScanSessionManager<JaipurHomewareVendor> sessionManager, IPageFetcher<JaipurHomewareVendor> pageFetcher)
        {
        ...
        {
            // showing 32 per page
            var totalResults = Convert.ToInt32(page.InnerText.CaptureWithinMatchedPattern(@"Total Results:\s+(?<capture>(\d+))"));
        ...
        public async Task<List<string>> GetProductIdsAsync(string searchUrl, HtmlNode firstPage, string taskName, string prefix)
        {
            ...
            var lastId = 0;
            await _sessionManager.ForEachNotifyAsync(taskName, Enumerable.Range(1, numPages), async i =>
            {
        ...
        {
            return string.Format("http://www.jaipurrugs.com/scroll.aspx?qstr=where GroundColorFamily IN ('{0}') AND ProductType in ('Rugs') @3295@0@78&productId={{0}}", color);
        }

        private string CreatePatternSearchUrl(string pattern)
        {
            return string.Format("http://www.jaipurrugs.com/scroll.aspx?qstr=where Pattern IN ( '{0}' ) AND ProductType in ('Rugs') @3295@0@78&productId={{0}}", pattern);
        }

        private string CreateDesignerSearchUrl(string designer)
        {
            return string.Format("http://www.jaipurrugs.com/scroll.aspx?qstr=where Designer IN ( '{0}' ) AND ProductType in ('Rugs') @3295@0@78&productId={{0}}", designer);
        }

        public async Task<List<string>> RunColorSearch(string color)
        {
            var firstPage = await _pageFetcher.FetchAsync(string.Format("http://www.jaipurrugs.com/rugs.aspx?p5={0}", color), CacheFolder.Search, color);
        ...
        {
            var firstPage = await _pageFetcher.FetchAsync(string.Format("http://www.jaipurrugs.com/rugs.aspx?p8={0}", pattern.Replace(" ", "_")), CacheFolder.Search, pattern);
        ...
        {
            var firstPage = await _pageFetcher.FetchAsync(string.Format("http://www.jaipurrugs.com/rugs.aspx?p10={0}", designer.Replace(" ", "_")), CacheFolder.Search, designer);
