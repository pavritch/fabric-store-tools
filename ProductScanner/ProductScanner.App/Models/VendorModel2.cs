...
namespace ProductScanner.App
{
    ...
    /// </summary>
    public partial class VendorModel : ObservableObject
    {
        #region Local Data
        /// <summary>
        ...
        protected async Task RefreshCheckpointDetails()
        {
            ...
            if (cp != null)
            {
                ...
                _session.ReplaceLog(cp.LogEvents);
            ...
            else
            {
                ...
                DelayMSBetweenVendorWebsiteRequests = await _session.GetThrottleAsync();
                AddLog("Ready.");
        ...
        protected async Task RefreshVendorProperties()
        {
            ...
            Status = vp.Status;
            VendorWebsiteUsername = Vendor.Username;
            VendorWebsitePassword = Vendor.Password;
        }
        ...
        protected async Task RefreshRecentCommits()
        {
            ...
            var list = await dbScanner.GetCommitBatchSummariesAsync(Vendor.Id, 0, 6);
        ...
        {
            var dbScanner = App.GetInstance<IScannerDatabaseConnector>();
            var cp = await dbScanner.GetCheckpointDetailsAsync(Vendor);
            return cp;
        }
        ...
        {
            var dbScanner = App.GetInstance<IScannerDatabaseConnector>();
            return await dbScanner.GetPendingCommitBatchCountAsync(Vendor.Id);
        }


        protected async Task<ProductScanner.Core.DataInterfaces.ProductCountMetrics> GetProductCountMetrics()
        {
            var dbStore = App.GetInstance<IStoreDatabaseConnector>();
            var metrics = await dbStore.GetProductCountMetricsAsync(ParentStore.Key, Vendor.Id);
            return metrics;
        }
        ...
        {
            var dbScanner = App.GetInstance<IScannerDatabaseConnector>();
            return await dbScanner.GetPendingCommitBatchSummariesAsync(Vendor.Id);
        }
        ...
        public async Task<bool> BeginTestingAsync()
        {
            if (IsScanning)
                return false;

            var vendorTester = App.GetInstance(typeof(IVendorTestRunner<>).MakeGenericType(Vendor.GetType())) as IVendorTestRunner;
            // need to run the auth test to get cookies so we can run the remaining tests
            var authResult = await vendorTester.RunAuthTest();
            _currentCookies = authResult.Cookies;
        ...
        {
            var dbScanner = App.GetInstance<IScannerDatabaseConnector>();
            var props = await dbScanner.GetVendorPropertiesAsync(Vendor.Id);
            return props;
        }
        ...
        {
            var dbScanner = App.GetInstance<IScannerDatabaseConnector>();
            return dbScanner.GetCommitBatchSummariesAsync(Vendor.Id, skip, take);
        }
        ...
        {
            var dbScanner = App.GetInstance<IScannerDatabaseConnector>();
            var result = await dbScanner.SaveVendorPropertiesAsync(Vendor.Id, properties);
        ...
        public ObservableCollection<CommitBatchSummary> RecentCommits
        {
            ...
            private set
            {
                ...
                // also need to update the last value
                RaisePropertyChanged(() => LastCommitDate);
            }
        }
        ...
        public int MaximumScanningErrorCount
        {
            ...
            set
            {
                ...
                _session.MaximumScanningErrorCount = value;
            }
        }
        ...
        public int DelayMSBetweenVendorWebsiteRequests
        {
            ...
            set
            {
                ...
                _session.UserDefinedThrottle = value;
            }
        }
        ...
        /// Null when no operation or nothing to report.
        /// </remarks>
        private int _scanningProgressErrorCount = 0;
        public int ScanningProgressErrorCount
        {
            get
            {
                return _scanningProgressErrorCount;
            }
            private set
            {
                Set(() => ScanningProgressErrorCount, ref _scanningProgressErrorCount, value);
            }
        }
        ...
        protected async Task StartedScanning()
        {
            ...
            if (ScanningOptions.Count > 0)
            {
                foreach (var item in ScanningOptions)
                    AddLog(string.Format("     {0}", item.DescriptionAttr()));
            }
            else
            {
                AddLog("     None");
            ...
            Debug.WriteLine(string.Format("StartedScanning: {0}", this.Name));
        ...
        protected void OneSecondInterval()
        {
            ...
            ScanningDuration = _session.GetTotalDuration();

            // update requests per minute
            var rpm = _session.ScanningRequestsTimeline.SecondsTimeline.Sum();
        ...
        /// Will send messages, etc.
        /// </remarks>
        protected async Task CancelledScanningAsync()
        {
            await DispatcherHelper.RunAsync(async () =>
                {
                    ...
                    LastCheckpointDate = null;
                    AddLog(string.Format("Cancelled scanning: {0}", DateTime.Now));
                    AddLog(string.Format("Duration: {0}", ScanningDuration.GetValueOrDefault().ToString(@"d\.hh\:mm\:ss")));

                    // the notification goes out last, since recipients will query various properties.
                    await SendScanningOperationNotification(ScanningOperationEvent.Cancelled);
                    Debug.WriteLine(string.Format("CancelledScanningAsync: {0}", this.Name));
        ...
        protected async Task ResumedScanning()
        {
            ...
            await RefreshCheckpointDetails();
            AddLog("Options:");
            ...
            ScannerState = DetermineScannerState();
            AddLog(string.Format("Resumed scanning: {0}", DateTime.Now));

            // the notification goes out last, since recipients will query various properties.
            await SendScanningOperationNotification(ScanningOperationEvent.Resumed);

            IsSuspended = false;
            Debug.WriteLine(string.Format("ResumedScanning: {0}", this.Name));
        ...
        /// Will send messages, etc.
        /// </remarks>
        protected async Task SuspendedScanningAsync()
        {
            await DispatcherHelper.RunAsync(async () =>
                {
                    RecalculateCachedFilesMetrics();
                    ...
                    ScanningRequestsPerMinute = 0;
                    ClearProgressMeter();
                    AddLog(string.Format("Suspended scanning: {0}", DateTime.Now));
                    AddLog(string.Format("Duration: {0}", ScanningDuration.GetValueOrDefault().ToString(@"d\.hh\:mm\:ss")));

                    // the notification goes out last, since recipients will query various properties.
                    await SendScanningOperationNotification(ScanningOperationEvent.Suspended);
                    Debug.WriteLine(string.Format("SuspendedScanningAsync: {0}", this.Name));

                });
        }

        protected async Task SubmittedCommitBatchAsync()
        {
            await DispatcherHelper.RunAsync(async () =>
                {
                    ...
                    Debug.WriteLine(string.Format("SubmittedCommitBatchAsync: {0}", this.Name));
        ...
        protected async Task FinishedScanningAsync()
        {
            await DispatcherHelper.RunAsync(async () =>
                {
                    RecalculateCachedFilesMetrics();
                    ...
                    AddLog(string.Format("Finished scanning: {0}", DateTime.Now));
                    AddLog(string.Format("Errors: {0:N0}", ScanningErrorCount));
                    AddLog(string.Format("Duration: {0}", ScanningDuration.GetValueOrDefault().ToString(@"d\.hh\:mm\:ss")));
                    ...
                    Debug.WriteLine(string.Format("FinishedScanningAsync: {0}", this.Name));
                });
        }
        ...
        /// </remarks>
        protected async Task FailedScanningAsync()
        {
            await DispatcherHelper.RunAsync(async () =>
                {
                    RecalculateCachedFilesMetrics();
                    ...
                    AddLog(string.Format("Failed scanning: {0}", DateTime.Now));
                    AddLog(string.Format("Errors: {0:N0}", ScanningErrorCount));
                    AddLog(string.Format("Duration: {0}", ScanningDuration.GetValueOrDefault().ToString(@"d\.hh\:mm\:ss")));
                    ...
                    Debug.WriteLine(string.Format("FailedScanningAsync: {0}", this.Name));
        ...
        private async Task AddScanningLogEventAsync(EventLogRecord record)
        {
            await DispatcherHelper.RunAsync(async () =>
            {
                ScanningLogEvents.Add(record);

                // if it's an error - then update other indicators in UX accordingly

                if (record.EventType == EventType.Error)
                {
                    // do not overwrite any existing warning.

                    if (!HasWarning)
                        SetWarning(record.Text);
        ...
        {
            await DispatcherHelper.RunAsync(() => ScanningLogEvents.Clear());
        }

        private void AddLog(string text, EventType eventType = EventType.General)
        {
            var logEntry = new EventLogRecord(eventType, text);
            _session.Log(logEntry);
            scanningLogHasData = true;
        }

        private async Task CreatedScanningCheckpointAsync()
        {
            await DispatcherHelper.RunAsync(async () =>
                {
                    LastCheckpointDate = DateTime.Now;
                    AddLog(string.Format("Created checkpoint: {0}", DateTime.Now));
                    await SendScanningOperationNotification(ScanningOperationEvent.CreatedCheckpoint);
                    Debug.WriteLine(string.Format("CreatedScanningCheckpointAsync: {0}", this.Name));
                    RecalculateCachedFilesMetrics();
                });
        } 
        ...
        public bool IsVariantCentricInventory
        {
            get
            {
                // TODO: would be better to do this individually by vendor - have the vendor module indicate its preference.
