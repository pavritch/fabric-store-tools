...
namespace Website
{
    [ProductFeed(ProductFeedKeys.Shopzilla)]
    public class ShopzillaProductFeed : ProductFeed, IProductFeed
    {
        public ShopzillaProductFeed(IStoreProductFeed storeProductFeed)
            : base(ProductFeedKeys.Shopzilla, storeProductFeed)
        {
        }
        ...
        /// </remarks>
        public Dictionary<string, ShopzillaFeedProduct> Products { get; private set; }

        #endregion

        public void AddProduct(FeedProduct feedProduct)
        {
            var product = feedProduct as ShopzillaFeedProduct;

            if (!Products.TryAdd(product.ID, product))
                throw new Exception(string.Format("Duplicate Shopzilla feed product ID {0} for {1}.", product.ID, Store.StoreKey));
        }
        ...
        protected int CountDuplicateImageLinks()
        {
            ...
            var dic = new Dictionary<string, ShopzillaFeedProduct>();
        ...
        {
            Products = new Dictionary<string, ShopzillaFeedProduct>();
        }
        ...
        {
            WriteFieldNamesHeader<ShopzillaFeedProduct>(file);
