...
namespace Website
{
    [ProductFeed(ProductFeedKeys.Pronto)]
    public class ProntoProductFeed : ProductFeed, IProductFeed
    {

        public ProntoProductFeed(IStoreProductFeed storeProductFeed)
            : base(ProductFeedKeys.Pronto, storeProductFeed)
        {
        }
        ...
        /// </remarks>
        public Dictionary<string, ProntoFeedProduct> Products { get; private set; }

        #endregion

        public void AddProduct(FeedProduct feedProduct)
        {
            var product = feedProduct as ProntoFeedProduct;

            if (!Products.TryAdd(product.ProductSKU, product))
                throw new Exception(string.Format("Duplicate Pronto feed product ID {0} for {1}.", product.ProductSKU, Store.StoreKey));
        }
        ...
        protected int CountDuplicateImageLinks()
        {
            ...
            var dic = new Dictionary<string, ProntoFeedProduct>();
            ...
            foreach (var product in Products.Values)
            {
                ...
                if (!dic.TryAdd(key, product))
                {
                    ...
                    Debug.WriteLine(string.Format("Duplicate image link for SKU {0} and {1}: {2}", product.ProductSKU, existingProduct.ProductSKU, product.ImageURL));
                    countDuplicates++;
                }
            }
        ...
        {
            Products = new Dictionary<string, ProntoFeedProduct>();
        }
        ...
        {
            WriteFieldNamesHeader<ProntoFeedProduct>(file);
