...
namespace Website
{
    [ProductFeed(ProductFeedKeys.Nextag)]
    public class NextagProductFeed : ProductFeed, IProductFeed
    {
        public NextagProductFeed(IStoreProductFeed storeProductFeed)
            : base(ProductFeedKeys.Nextag, storeProductFeed)
        {
        }
        ...
        /// </remarks>
        public Dictionary<string, NextagFeedProduct> Products { get; private set; }

        #endregion

        public void AddProduct(FeedProduct feedProduct)
        {
            var product = feedProduct as NextagFeedProduct;

            if (!Products.TryAdd(product.ID, product))
                throw new Exception(string.Format("Duplicate Nextag feed product ID {0} for {1}.", product.ID, Store.StoreKey));
        }
        ...
        protected int CountDuplicateImageLinks()
        {
            ...
            var dic = new Dictionary<string, NextagFeedProduct>();
        ...
        {
            Products = new Dictionary<string, NextagFeedProduct>();
        }
        ...
        {
            WriteFieldNamesHeader<NextagFeedProduct>(file);
