...
namespace Website
{
    public class BookProductCollectionUpdater : ProductCollectionUpdaterBase
    {

        private string mostUsedLabel = null;

        public BookProductCollectionUpdater(IWebStore store, int manufacturerID)
        ...
        {
            return FetchExistingCollections(0); // 0-Books
        }

        /// <summary>
        /// Creates a dic with key as book name, with list of member products.
        /// </summary>
        /// <returns></returns>
        protected override Dictionary<string, List<MemberProduct>> FetchGroupedProducts()
        {
            ...
            try
            {
                var labelValues = (from pl in DataContext.ProductLabels
                                   where pl.Label == mostUsedLabel
                                   join pm in DataContext.ProductManufacturers on pl.ProductID equals pm.ProductID
                                   where pm.ManufacturerID == ManufacturerID
                                   join p in DataContext.Products on pm.ProductID equals p.ProductID
                                   where supportedGroups.Contains(p.ProductGroup)
            ...
            var dic = (from pl in DataContext.ProductLabels where pl.Label == mostUsedLabel
                        ...
                        select new
                        {
                            Book = grp.Key,
                            Items = grp.ToList(),
                        }).ToDictionary(k => k.Book.ToUpper(), v => v.Items);
        ...
        protected override string DisplayPrefix
        {
            get
            {
                return string.Format("{0} books...", CleanManufacturerName);
            }
        }
        ...
        {
            base.Initialize();
            mostUsedLabel = "Book"; // GetMostUsedLabel(new string[] { "Book", "Collection" });
        }

        private string GetPureName(string key, List<MemberProduct> members)
        {
            // key is name of book, but is upper case - so need to go get the mixed case name

            var memberCollection = members.Select(e => e.ProductID).Take(100).ToList();
            var names = (from pl in DataContext.ProductLabels
                         where pl.Label == "Book" && pl.Value == key && memberCollection.Contains(pl.ProductID)
                         select pl.Value).Distinct().ToList();

            if (names.Count() == 0)
                return key; // will be upper case, but can't be choosey here

            // save the first name, it will be mixed case, but sometimes might contain a comma if in more than one book
        ...
        {
            var name = string.Format("{1} {2} Book {0}", pureName, CleanManufacturerName, productGroup);
            return name;
        }
        ...
        protected override ProductCollection MakeNewCollectionRecord(string key)
        {
            ...
            else
            {
                ...
                rec.SEDescription = string.Format("Lowest prices on {1} {2} book {0}. Shop thousands of patterns. Fast delivery. Free shipping!", pureName, CleanManufacturerName, rec.ProductGroup.ToLower());
            }
        ...
        {
            var filename = string.Format("{0}-{1}-{2}.jpg", "book", rec.ProductCollectionID, rec.SEName);
